    ...
    const response = await fetch(`${openAIConfig.base_url}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: 'You are an assistant that provides concise summaries of articles in 3-5 bullet points highlighting the key points.' 
          },
          { 
            role: 'user', 
            content: prompt 
        ...
        <div class="flex items-start">
          ...
          <div class="flex-1">
            <div class="flex justify-between items-start">
              ...
              >
                ${!openAIConfig.api_key ? 'Config Needed' : 'AI Summary'}
              </button>
            </div>
            <div id="ai-summary-${article.id}" class="ai-summary-content hidden mt-2 p-2 bg-gray-100/50 rounded-lg text-xs text-gray-700"></div>
          </div>
        </div>
      </div>
    `;
  ...
  async handleAISummary(articleId) {
    ...
    try {
      ...
      if (!content && article.url) {
        // 对于外部链接，我们只能使用标题和简单描述
        content = `This is an external link: ${article.url}. ${article.title}`;
      }
