...
class OpenAIClient {
  static async getArticleSummary(content, title) {
    ...
    return data; // 返回包含summary和model的完整数据
  }
}
...
class HackerNewsApp {
  ...
  async handleAISummary(articleId) {
    ...
    try {
      ...
      // 调用OpenAI API获取摘要
      const summaryData = await OpenAIClient.getArticleSummary(
        content,
        article.title,
      );
      summary = summaryData.summary;
      const modelUsed = summaryData.model || openAIConfig.model; // 使用后端实际使用的模型，或fallback到前端配置
      ...
      summaryContainer.innerHTML = `
        ...
        <div class="ai-summary-text">${ArticleRenderer.formatSummaryAsHTML(summary)}</div>
        <div class="mt-2 text-[0.5rem] text-gray-500 italic">Generated by ${modelUsed}</div>
      `;
