...
class Database {
  ...
  async articleExists(hnId) {
    return new Promise((resolve, reject) => {
      this.db.get(
        "SELECT hn_id FROM articles WHERE hn_id = ?",
        [hnId],
        (err, row) => {
          if (err) {
            reject(err);
  ...
  async saveAISummary(articleId, summary, model) {
    const insertQuery =
      "INSERT INTO ai_summaries (article_id, summary, model) VALUES (?, ?, ?)";

    return new Promise((resolve, reject) => {
      this.db.run(insertQuery, [articleId, summary, model], function (err) {
        if (err) {
          console.error("Database error in saveAISummary:", err);
          reject(err);
        } else {
          // 安全地访问lastID，避免潜在的问题
  ...
  async getAISummary(articleId) {
    return new Promise((resolve, reject) => {
      this.db.get(
        "SELECT summary, model, created_at FROM ai_summaries WHERE article_id = ?",
        [articleId],
        (err, row) => {
          if (err) {
            reject(err);
